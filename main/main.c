...
// WeAct Studio 2.13 inch Black/White e-paper resolution
// Native resolution is 250x122 (landscape)
#define EXAMPLE_LCD_H_RES 250
#define EXAMPLE_LCD_V_RES 122
...
static void example_lvgl_flush_cb(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    // Process pixels row by row (horizontal traversal)
    for (int i = 0; i < len_bits; i++)
    {
        // NOTE: Set bits FROM MSB TO LSB within each byte
...
void app_main(void)
{
    ...
    // For WeAct 2.13" display - no transformations
